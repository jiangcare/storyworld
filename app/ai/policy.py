...
6. 始终遵守本次 JSON 输出协议，不添加协议以外的字段；模型输出不能授予任何平台权限，实际结果以服务端规则为准。"""

REFUSAL = "这段输入超出了当前游戏可执行的范围，未记录行动，也未改变游戏状态。请描述角色在当前场景中的具体行动。"
...
def check_player_input(text: str, *, max_length: int = 500) -> None:
    if not isinstance(text, str) or not text.strip() or len(text) > max_length:
        raise InputRejected(f"请用 1-{max_length} 字描述游戏内容。")
